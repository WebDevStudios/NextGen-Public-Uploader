...
if ( !class_exists( "npuGalleryUpload" ) ) {
	...
	class npuGalleryUpload {
		...
		public function npu_upload_control( $widget_args = 1 ) {
			...
			if ( !$updated && !empty( $_POST['sidebar'] ) ) {
				...
				foreach ( (array)$_POST['widget_npu_upload'] as $widget_number => $widget_npu_upload ) {
					...
					$options[$widget_number]['gal_id'] = explode(',', $widget_npu_upload['gal_id'] );
				}
			...
			<p>
				<label for="npu_upload-id-<?php echo $number; ?>"><?php _e( 'Upload to (gallery ID, separated by comma):', 'nggallery' ); ?></label><br/>
				<input type="text" id="npu_upload-id-<?php echo $number; ?>" name="widget_npu_upload[<?php echo $number; ?>][gal_id]" value="<?php echo ( is_array( $options[$number]['gal_id'] )) ? implode(',',$options[$number]['gal_id']) : $options[$number]['gal_id']; ?>">
			</p>
		...
		public function display_uploader( $gal_ids, $strDetailsPage = false, $blnShowAltText = true, $echo = true ) {
			$strOutput = "";
		...
		public function display_uploader_widget( $gal_ids, $strDetailsPage = false, $blnShowAltText = true, $echo = true ) {
			$strOutput = "";
		...
		public function handleUpload() {
			...
			if ( isset( $_POST['uploadimage'] ) ) {
				...
				if ( count( $this->arrErrorMsg ) > 0 && ( is_array( $this->arrImageIds ) &&count( $this->arrImageIds ) > 0 ) ) {
					if ( !empty( $_POST['galleryselect'] ) ) {
						foreach ( $_POST['galleryselect'] as $gal_id ) {
		...
		public function shortcode_show_uploader( $atts ) {
			...
			$ids = explode( ',', $id );
			$this->handleUpload();
			$out = $this->display_uploader( $ids, false, true, false );
			return $out;
		}
